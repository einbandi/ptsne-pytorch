...
class Visualiser():
    ...
    def plot_table_html(self, x, y, key, split_name, **kwargs):
        ...
        y_data = y['data']
        # table = pd.DataFrame(np.array(y_data).transpose(), index=kwargs['labels'], columns=y['colnames'])
        # modified by Lorna
        table = pd.DataFrame(np.vstack(y_data).transpose(), index=kwargs['labels'], columns=y['colnames'])
        # print(table)
